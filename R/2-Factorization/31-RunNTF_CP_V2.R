...
rm(list = ls())
load("temp/31-v2-data.RData")
loadls("ThreeWay",FALSE)
...
LoadMyData <- funtion()
{
  ...
  load("temp/20-1and2_86clinicalBRCA_v2.RData")
  rm(list=ls()[-which(ls() %in% c("boolClinical","MatrixOrderPxGxC","loadls","loadlib",
                                  "boolClinicalNoNames","boolMutation","boolMutationNoNames"))])


  loadls("ThreeWay",FALSE) #For tensors

  save.image(file="temp/31-v2-data.RData")
}


runCP <- function(MatrixOrderPxGxC)
{
  labelsPA  = row.names(boolMutation)
  labelsGenes =  names(boolMutation)
  labelsClinical = names(boolClinical)

  # >   dim(MatrixOrderPxGxC)
  # [1]     507 1068267
  # >   length(labelsPA)
  # [1] 507
  # >   length(labelsGenes) *  length(labelsClinical)
  # [1] 1068267
  # > length(labelsPA)
  # [1] 507
  # > length(labelsGenes)
  # [1] 12003
  # > length(labelsClinical)
  # [1] 89
