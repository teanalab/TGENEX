...
#Dichomotization of columns with more than 2 categories
dichoSeveralCols <-function(cols, logicVal=TRUE )
{
...
classesSturges <- function(colId,N=0)
{
  require(classInt)
  if(N==0){
    aInt <- classIntervals(as.numeric(as.character(clinical[patients,colId])), style = "jenks")
  } else {
    aInt <- classIntervals(as.numeric(as.character(clinical[patients,colId])), n=N, style = "jenks")
  }
