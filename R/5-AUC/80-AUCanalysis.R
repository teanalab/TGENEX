...
save.image(file="temp/8_components15.RData")
...
#coxphModel <- coxph(Surv(obsPeriod,label)~age+sex+race+lungCH+COPDH+chestXray3+smokerAge+smokingAmount+
#          quitTime+duration+packYears+status+emphysema+BMI, x=TRUE, y=TRUE, method = "breslow", data=train)
...
# Concordance= 0.619  (se = 0.054 )
# Rsquare= 0.031   (max possible= 0.639 )
# Likelihood ratio test= 12.06  on 8 df,   p=0.1487
# Wald test            = 20.62  on 8 df,   p=0.008218
# Score (logrank) test = 100.6  on 8 df,   p=0


coxFit <- coxph(Surv(time = Overall.Survival..Months., event = Overall.Survival.Status)~Converted.Stage+
                  Diagnosis.Age+ER.Status+HER2.Status+Metastasis.Coded+PR.Status+Tumor..T1.Coded,
                x=TRUE, y=TRUE, method = "breslow", data=train)
summary(coxFit)
...
coxFit2 <- coxph(Surv(time = Overall.Survival..Months., event = Overall.Survival.Status)~Converted.Stage+
                  Diagnosis.Age+ER.Status+HER2.Status+Metastasis.Coded+PR.Status+
                  Tumor..T1.Coded+component,
                x=TRUE, y=TRUE, method = "breslow", data=train)
summary(coxFit2)
...
#AUC2
lp <- predict(coxFit2)
lpnew <- predict(coxFit2, newdata=test)
Surv.rsp <- Surv(time = train$Overall.Survival..Months., event = train$Overall.Survival.Status)
Surv.rsp.new <- Surv(time = test$Overall.Survival..Months., event = test$Overall.Survival.Status)
times <- seq(0.74, 221, 0.74)
AUC_CD <- AUC.cd(Surv.rsp, Surv.rsp.new, lp, lpnew, times)
