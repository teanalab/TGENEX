...
{
  prefixPatAfilFile <- "data/patientA_NMF_M/patientA_NMF_M_"
  ouputFileEachK <- "temp/3v6-AUC_NMF_PxM_k"
  folderOutput <- "output4paper/NMF_PxM_"
  kMax=12
  numSplits = 10
  ...
  for (k in c(2:kMax) ){
    ...
    set.seed(k*123)  # set seed for reproducibility
  ...
  save(NMF_PxM_AUC, file = paste(folderOutput, "AUC.RData", sep='') )
