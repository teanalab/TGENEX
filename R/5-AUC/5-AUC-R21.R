...
# {
  ...
  sdX <- apply(x,2,sd)
  NTF_AUC_ = data.frame(k=interval, auc= meanX, method = "NTF", sd= sdX)

  x = as.data.frame(NMF_PxM_AUC[interval])
  meanX <- apply(x,2,mean)
  sdX <- apply(x,2,sd)
  NMF_PxM_AUC_ = data.frame(k=interval, auc= meanX, method = "NMF mutations", sd= sdX)

  x = as.data.frame(NMF_PxC_AUC[interval])
  meanX <- apply(x,2,mean)
  sdX <- apply(x,2,sd)
  NMF_PxC_AUC_ = data.frame(k=interval, auc= meanX, method = "NMF clinical", sd= sdX)

  x = as.data.frame(random_AUC[interval])
  meanX <- apply(x,2,mean)
  sdX <- apply(x,2,sd)
  randomAUC_ = data.frame(k=interval, auc= meanX, method = "random", sd= sdX)
#}
...
#WithRandomAsBaseline <- function(){
  ...
  g1 <- ggplot(data10, aes(x=k, y=auc, colour=method, shape=method, fill=method)) +
    ...
    theme(legend.background = element_rect(linetype = "solid", colour = "black", size=0.25))
  g1 + scale_x_continuous(name="number of breast cancer sub-types", breaks = (2:kMax))
